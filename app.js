...
(() => {
  ...
  function buildRow(entry) {
    ...
    const timeInput = createInput('text', 'input-small', 'HH:MM', entry.time);
  ...
  function validateTime(input) {
    ...
    const valid = /^\d{1,3}:\d{2}$/.test(val);
    if (valid) {
      const [, mm] = val.split(':').map(Number);
      if (mm >= 60) { input.classList.add('input-invalid'); return false; }
    }
  ...
  /** Parse "HH:MM" → total minutes, or null */
  ...
  /** Format minutes exactly but show fractional minutes as decimal (for averages) */
  function formatAvg(totalMin) {
    if (totalMin === null || isNaN(totalMin)) return '—';
    const h = Math.floor(totalMin / 60);
    const m = totalMin % 60;
    const mRound = Math.round(m * 10) / 10;
    if (h === 0) return `${mRound}m`;
    return `${h}h ${String(Math.round(m)).padStart(2, '0')}m`;
  }
  ...
  function recalculate() {
    ...
    entries.forEach(entry => {
      const mins = parseTime(entry.time);
      if (mins !== null) { totalMinutes += mins; validTimeCount++; }
      const lv = parseFloat(entry.leveling) || 0;
      if (lv > 0) totalLeveling += lv;
    });

    const avgMin = totalLeveling > 0 ? totalMinutes / totalLeveling : null;

    const totalTimeStr = formatMinutes(totalMinutes);
    const avgStr = avgMin !== null ? formatAvg(avgMin) : '—';
  ...
  function updateRowAvg(tr, entry) {
    ...
    const lv = parseFloat(entry.leveling) || 0;
    if (mins !== null && lv > 0) {
      avgSpan.textContent = formatAvg(mins / lv);
